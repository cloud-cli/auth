...
New OIDC clients can be managed at `/oidc` by users listed in `OIDC_ADMIN_USER_IDS`. Secrets are generated by the server and shown only once. Existing `OIDC_CLIENTS` entries continue to work as a compatibility fallback.
