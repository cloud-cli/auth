...
export type QrLoginTransaction = {
  token: string;
  sessionId: string;
  returnUrl: string;
  createdAt: string;
...
function mapRow(table: string, row: Record<string, any>) {
  ...
  for (const [camel, column] of Object.entries(mapping || {})) {
    result[camel] =
      column === 'value' && row.value_type === 'json'
        ? decode(row[column])
...
export async function rows<T>(table: string, where = '', params: unknown[] = []) {
  return (await all<Record<string, any>>(`SELECT * FROM ${table}${where ? ` WHERE ${where}` : ''}`, params)).map(
    (row) => mapRow(table, row),
  ) as T[];
}
...
export async function saveAuthenticator(item: Authenticator) {
  await run(
    'INSERT OR REPLACE INTO auth_authenticator (credential_id, user_id, public_key, counter, transports, label, created_at, last_used_at, revoked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [
      item.credentialId,
      item.userId,
      item.publicKey,
      item.counter,
      json(item.transports),
      item.label,
      item.createdAt,
      item.lastUsedAt,
      item.revokedAt,
