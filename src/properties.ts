...
export async function setProperty(userId: string | undefined, key: string, value: string) {
  ...
  if (found.length) {
    ...
    await run('UPDATE auth_property SET value = ?, value_type = ? WHERE uid = ?', [
      String(value),
      'text',
      property.uid,
    ]);
  ...
  await run('INSERT INTO auth_property (uid, user_id, key, value, value_type) VALUES (?, ?, ?, ?, ?)', [
    uid,
    userId,
    key,
    String(value),
...
export async function getProperties(userId?: string, key?: string) {
  ...
  const entries = await rows<UserProperty>(
    'auth_property',
    key ? 'user_id = ? AND key = ?' : 'user_id = ?',
