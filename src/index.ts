...
app.get('/ui/:asset', (req, res) => {
  ...
  const source = req.params.asset === 'profile.html'
    ...
    : ['security.html', 'properties.html', 'activity.html', 'oidc-apps.html', 'keys.html', 'tokens.html'].includes(req.params.asset)
      ? asset.replaceAll("from '/dashboard.mjs'", `from '${dashboardUrl}'`).replaceAll("from '@apphor/dashboard.mjs'", `from '${dashboardUrl}'`).replace('const showTokenForm=', "const editingApp=ref(null),editingCallbackText=hook('');const beginEdit=(app)=>{editingApp.value=app;editingCallbackText.value=app.redirectUris.join('\\n')};const setEditingCallbackText=(value)=>editingCallbackText.value=value;const saveEdit=async()=>{const result=await updateOidcCallbacks(editingApp.value.id,editingCallbackText.value.split('\\n').map((value)=>value.trim()).filter(Boolean));editingApp.value.redirectUris=result.redirectUris;editingApp.value=null};const cancelEdit=()=>editingApp.value=null;const showTokenForm=").replace('addScope,showTokenForm,createToken', 'addScope,editingApp,editingCallbackText,setEditingCallbackText,beginEdit,saveEdit,cancelEdit,showTokenForm,createToken').replace('<summary class="cursor-pointer font-bold">{{ app.id }}</summary>', '<summary class="cursor-pointer font-bold">{{ app.id }}</summary><div class="mt-2 rounded-xl bg-violet/5 p-3 text-sm"><strong>Callback URLs</strong><template for="uri of app.redirectUris"><span class="mt-1 block break-all text-slate-600">{{ uri }}</span></template><template if="admin"><button class="mt-2 rounded-lg border border-violet px-3 py-1 font-semibold text-violet" on-click="beginEdit(app)">Edit callbacks</button><template if="editingApp === app"><div class="mt-3 grid gap-2"><textarea class="min-h-24 rounded-xl border border-slate-200 px-3 py-2" bind-value="editingCallbackText" on-input="setEditingCallbackText($event.target.value)"></textarea><div class="flex gap-2"><button class="rounded-lg bg-violet px-3 py-1 font-semibold text-white" on-click="saveEdit()">Save</button><button class="rounded-lg border border-slate-200 px-3 py-1" on-click="cancelEdit()">Cancel</button></div></div></template></template></div>').replace('mt-5 rounded-3xl bg-white p-6', 'mt-3 rounded-2xl bg-white p-4').replace('mt-5 grid gap-3', 'mt-3 grid gap-2').replace('mt-4 grid gap-5', 'mt-3 grid gap-3').replace('p-4" on-toggle', 'p-3" on-toggle')
      : asset;
